...
export const datasourceApi = {
  /** 列出当前用户全部启用数据源 */
  list: () => api.get<DatasourceResponse[]>('/datasources'),

  /** 新增数据源 */
  create: (data: DatasourceRequest) => api.post<DatasourceResponse>('/datasources', data),

  /** 更新数据源（passwordPlain 留空 = 不修改密码） */
  update: (id: string, data: DatasourceRequest) =>
    api.put<DatasourceResponse>(`/datasources/${id}`, data),
  ...
  testNew: (data: DatasourceRequest) =>
    api.post<{ success: boolean }>('/datasources/test', data),

  /** 复测已保存的数据源（可不传 passwordPlain；自动从 cipher 回源） */
  testExisting: (id: string, data?: Partial<DatasourceRequest>) =>
    api.post<{ success: boolean }>(`/datasources/${id}/test`, data ?? {}),
}
